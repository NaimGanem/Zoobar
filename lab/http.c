...
const char *http_request_line(int fd, char *reqpath, char *env, size_t *env_len)
{
    ...
    if (http_read_line(fd, buf, *env_len) < 0)
...
{
    static char buf[500];      /* static variables are not on the stack */
    int i;
    char value[512];
    char envvar[512];
...
void you_are_blocked(int fd) {
    int BUFFER_SIZE = 8192;
    int file_fd = open("blocked.html", O_RDONLY);
    if (file_fd == -1) {
        perror("open");
        return;
    }
